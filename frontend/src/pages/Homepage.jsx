...
export default function Homepage() {
  ...
  const rawSem = user?.semester || sem;
  const displaySem = rawSem || "Semester-3";
  const semKeyHyphen = displaySem.includes('-') ? displaySem : displaySem.replace(' ', '-');
  
  // Robust subject lookup matching user's registered semester
  const currentSubjects = subjectsData[semKeyHyphen] || 
    subjectsData[displaySem] || 
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-text-secondary text-base font-medium mt-1">
              {displaySem.replace('-', ' ')} • Branch: <span className="font-semibold text-slate-700">{displayBranch}</span>
            </p>
          </div>
        </div>
